...
void welgr() {
    ...
    setcolor(15);
    settextstyle(3, 0, 6);
    outtextxy((getmaxx() - textwidth("Systeme")) / 2, 1, "Systeme");
    outtextxy((getmaxx() - textwidth("d'information")) / 2, 70, "d'information");
    outtextxy((getmaxx() - textwidth("Hospitalier")) / 2, 140, "Hospitalier");
    settextstyle(3, 0, 4);
    outtextxy((getmaxx() - textwidth("par alnaser")) / 2, 250, "par alnaser");
    setfillstyle(1, LIGHTRED);

    int k;
    for (k = 0; k < 50; k++) {
        drawCircle(80, 80, k, RED);
    }
    for (k = 0; k < 40; k++) {
        drawCircle(100, 80, k, BLUE);
    }
    for (k = 0; k < 50; k++) {
        drawCircle(getmaxx() - 80, 80, k, RED);
    }
    for (k = 0; k < 40; k++) {
        drawCircle(getmaxx() - 100, 80, k, BLUE);
    }
...
void main() {
    ...
    outtextxy(midx, 0, "==== Systeme d'Information Hospitalier ====");
    ...
    while (1) {
        ...
        if (buttonm == 1) {
            if (xm >= 10 && xm <= 34 && ym >= 18 && ym <= 32) {
                ...
                menu1 = 3;
            } else if (xm >= 250 && xm <= 320 && ym >= 18 && ym <= 32) {
                menu1 = 4;
            } else menu1 = -1;
            clrwin(1);
            menu(menu1);
...
int program() {
    int opt = -1, j;
    char menu[2][20] = {"Connexion  >", "Quitter"};
    close = 1;
...
void userl() {
    int opt = -1, j;
    char menu[3][20] = {"Nouveau", "Changer", "Supprimer"};
...
void setusrl() {
    ...
    setcolor(15);
    moveto(midx / 2 + 15, midy - 26);
    outtext("Program Code :");
    inputbox(a, midy - 26, 15, 15);
    setcolor(15);
    moveto(midx / 2 + 15, midy - 7);
    outtext("Username     :");
    inputbox(a, midy - 7, 15, 15);
    setcolor(15);
    moveto(midx / 2 + 15, midy + 12);
    outtext("Password     :");
    inputbox(a, midy + 12, 15, 15);
    do {
        getmousepos(&buttonm, &xm, &ym);
        if (buttonm == 1) {
...
void changeusrl() {
    ...
    setcolor(15);
    moveto(midx / 2 + 15, midy - 26);
    outtext("Current Username :");
    inputbox(a, midy - 26, 15, 15);
    setcolor(15);
    moveto(midx / 2 + 15, midy + 12);
    outtext("Current Password :");
    inputbox(a, midy + 12, 15, 15);
    do {
        getmousepos(&buttonm, &xm, &ym);
        if (buttonm == 1) {
...
void notice() {
    ...
    settextstyle(7, 0, 4);
    setcolor(15);
    outtextxy(midx - ((17 / 2)*17), midy - 25, "Just Interface");
    settextstyle(0, 0, 0);
    do {
        getmousepos(&buttonm, &xm, &ym);
        if (buttonm == 1) {
...
int rooms() {
    char menu[4][20] = {"Chambres", "Disponible", "modifier    >", "Modif  type >"};
    int opt = -1, j;
    close = 1;
...
void manageroom() {
    int opt = -1, j;
    char menu[3][20] = {"Ajouter", "Supprimer", "Mis.Aj"};
...
void managetype() {
    int opt = -1, j;
    char menu[3][20] = {"Ajouter", "Supprimer", "tous type"};
...
int about() {
    char str[3][35] = {"Project : SIH Interface", "Created by : alnaser"};
    close = 1;
...
int doctor() {
    //    char menu[5][20] = {"Ajout", "Modifier", "Delete    >", "Search    >", "View"};
    char menu[5][20] = {"Ajout", "Modifier", "Supprimer >", "Recherche >", "Vue "};
    int opt = -1, j;
    close = 1;
...
void adddoc() {
    ...
    button("Cancel", midx - 90, midy * 3 / 2 - 22, midx - 30, midy * 3 / 2 - 8, 7);
    setcolor(15);
    moveto(a - textwidth("Program Code : "), midy - 26);
    outtext("Program Code :");
    inputbox(a, midy - 26, 15, 15);
    setcolor(15);
    moveto(a - textwidth("Program Code : "), midy - 7);
    outtext("Username     :");
    inputbox(a, midy - 7, 15, 15);
    setcolor(15);
    moveto(a - textwidth("Program Code : "), midy + 12);
    outtext("Password     :");
    inputbox(a, midy + 12, 15, 15);
    do {
        getmousepos(&buttonm, &xm, &ym);
...
void searchdoc() {
    int opt = -1, j;
    char menu[4][20] = {"Par ID", "Par Nom", "Par prenom", "Par specialiter"};
...
int patient() {
    char menu[5][20] = {"Entrer", "Sortir", "MisAjour  >", "Recheche  >", "Vue"};
    int opt = -1, j;
    close = 1;
...
void updatepat() {
    int opt = -1, j;
    char menu[3][20] = {"Tous", "Nom", "Chambre"};
...
void searchpat() {
    int opt = -1, j;
    char menu[5][20] = {"Par ID", "Par chambre", "Par nom", "par prenom", "par date"};
...
int menu(int select) {
    char ch, menu[5][20] = {"\360", "Medecin", "Patient", "Chambre", "A propos"};
    char date[11];
    struct date da;
    hidemouse();
    setfillstyle(1, 3);
